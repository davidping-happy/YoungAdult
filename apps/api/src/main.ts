...
async function bootstrap() {
  ...
  app.use((req: { method: string; path: string }, res: { json: (b: unknown) => void }, next: () => void) => {
    if (req.method === 'GET' && (req.path === '/' || req.path === '')) {
      return res.json({
        service: 'youngadult-api',
        message:
          '這是 API 服務，不是管理後台。請改開 https://youngadult-admin.onrender.com',
        health: '/api/health',
        livestream: '/api/livestream/latest',
        admin: 'https://youngadult-admin.onrender.com',
        prayer: 'https://youngadult-admin.onrender.com/prayer',
      });
    }
    return next();
  });
